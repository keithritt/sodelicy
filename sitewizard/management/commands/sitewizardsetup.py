...
# add parent dir to python path to allow for importing custom modules
...
class Command(BaseCommand):
  help = 'Runs the Site Setup Wizard'
  ...
  def handle(self, *args, **options):
    ...
    ****''')
    ...
    self.domain = Domain()
    self.domain.prompt_for_domain()
    print('domain set to: '+domain.domain)
    self.domain.save()
    ...
    File.force_folder_existance(self.path_base+self.domain.domain_code+'/')

    env = 'dev' #todo - need to move the to a config file
    self.git = Git(self.domain.code, env)
    self.git.run_wizard()
    ...
    if not File.exists(log_conf_file):
      text = '''
        ...
        autorestart=true
        redirect_stderr=true'''.format(domain_code=self.domain.code)
    ...
    text = '''
      ...
      sudo supervisorctl start {}'''.format(self.domain.code)
